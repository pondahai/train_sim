...
- **其他控制：**
    ...
    - 可切換軌道是否循環。
- **效能優化：**
    - **視錐體剔除 (Frustum Culling)：** 僅渲染視野內的物件，大幅提升效能。
    - **高效渲染：** 使用 **VBO/VAO** 與 **GLSL Shaders** 優化軌道、建築與山丘的繪製。
- **右鍵選單：**
    - 支援滑鼠右鍵開啟選單，可快速 **載入場景** 或 **離開程式**。
...
| frustum_culling.py   | 視錐體剔除邏輯，判斷物件是否在視野內                  |
| shaders_inline.py    | 內嵌 GLSL Shader 程式碼 (山丘、建築等)                |
...
- 整合 **Frustum Culling** 以優化渲染效能。
- 使用 **VBO/VAO** 與 **Shaders** 繪製複雜物件（如山丘、建築）。
...
### frustum_culling.py
- 實作 `Frustum` 類別，從 OpenGL 矩陣提取視錐體平面。
- 提供 `is_point_visible` 與 `is_sphere_visible` 方法，用於剔除視野外的物件。

### shaders_inline.py
- 儲存 GLSL Vertex 和 Fragment Shader 的原始碼字串。
- 目前包含用於渲染山丘 (`HILL_...`) 和建築 (`BUILDING_...`) 的 Shader。
...
- **點擊滑鼠右鍵**：開啟系統選單（載入場景/離開），同時會暫時解鎖滑鼠
- **Esc**：退出模擬器（或解鎖滑鼠）
